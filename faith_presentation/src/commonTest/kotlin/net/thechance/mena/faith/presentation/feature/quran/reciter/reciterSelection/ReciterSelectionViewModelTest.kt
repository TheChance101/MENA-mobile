...
class ReciterSelectionViewModelTest {
    ...
    @Test
    fun `init should load default reciter`() = runTest {
        val defaultReciterId = 2
        everySuspend { quranRepository.getDefaultReciter() } returns flowOf(defaultReciterId)

        val newVm = ReciterSelectionViewModel(quranRepository, searchRecitersUseCase, testDispatcher)
        testDispatcher.scheduler.advanceUntilIdle()
        assertEquals(defaultReciterId, newVm.uiState.value.selectedReciterId)
    }
