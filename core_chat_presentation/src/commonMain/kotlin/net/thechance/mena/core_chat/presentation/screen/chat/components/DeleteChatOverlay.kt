...
fun ScaffoldScope.deleteChatOverlay(
    showDeleteChatDialog: Boolean,
    onDeleteChatClicked: () -> Unit,
    onConfirmDeleteChatClicked: () -> Unit,
    onDismissDeleteChatDialog: () -> Unit,
    showConfirmDeleteChatDialog: Boolean,
) {
    dialog(isVisible = showConfirmDeleteChatDialog || showDeleteChatDialog) {
        BasicDialog(
            isVisible = showConfirmDeleteChatDialog || showDeleteChatDialog,
            onDismiss = onDismissDeleteChatDialog,
            onCancelClick = onDismissDeleteChatDialog,
            contentPadding = PaddingValues(12.dp),
            actionButtons = {
                if (showDeleteChatDialog) DeleteActionMenuItem(onDeleteChatClicked)
            }
        ) {
            Column {
                if (showDeleteChatDialog) DeleteChatContent()
                if (showConfirmDeleteChatDialog) ConfirmDeleteChatContent(onConfirmDeleteChatClicked)
            }
        }
    }
}
...
@Composable
private fun DeleteActionMenuItem(onDeleteChatClicked: () -> Unit){
    ActionMenuItem(
